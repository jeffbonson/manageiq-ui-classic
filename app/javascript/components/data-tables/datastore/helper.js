...
const nsListHeaders = (hasOptions) => {
  const columns = [
    ...
    { text: 'Name', header_text: 'Name' },
    { text: 'Description', header_text: 'Description' },
    { text: 'Enabled', header_text: 'Enabled' },
  ];
  if (hasOptions) columns.push({ text: 'Tenant', header_text: 'Tenant' });
  return columns;
};
...
const domainOverridesHeaders = () => [{ text: 'defaultKey_0', header_text: 'Domain' }];

/** Function which returns the header data for table with type class_fields schema. */
const schemaHeaders = () => [
  { text: 'Name', header_text: 'Name' },
  { text: 'Description', header_text: 'Description' },
  { text: 'DefaultValue', header_text: 'Default Value' },
  { text: 'Collect', header_text: 'Collect' },
  { text: 'Message', header_text: 'Message' },
  { text: 'OnEntry', header_text: 'On Entry' },
  { text: 'OnExit', header_text: 'On Exit' },
  { text: 'OnError', header_text: 'On Error' },
  { text: 'MaxRetries', header_text: 'Max Retries' },
  { text: 'MaxTime', header_text: 'Max Time' },
];

/** Function which returns the header data for table with type instant_fields. */
const instantFieldHeaders = (hasOptions) => {
  const columns = [{ text: 'Name', header_text: 'Name' }, { text: 'Value', header_text: 'Value' }];
  if (hasOptions) {
    [
      { text: 'OnEntry', header_text: 'On Entry' },
      { text: 'OnExit', header_text: 'On Exit' },
  ...
  [{ text: 'Collect', header_text: 'Collect' }, { text: 'Message', header_text: 'Message' }].map((item) => columns.push(item));
  return columns;
};
